...
def filter_active_bc_scope():
    ...
    print("First, let's analyze what values exist in ind_bc_scope...")
    ...
    try:
        ...
        with open(input_file, mode='r', encoding='utf-8', newline='') as fin:
            ...
            with open(temp_path, mode='w', encoding='utf-8', newline='') as fout:
                ...
                for row in reader:
                    initial_count += 1
                    scope_value = row['ind_bc_scope']
                    scope_values[scope_value] += 1
                    ...
                    if initial_count % 100000 == 0:
                        ...
                        # Show distribution of values so far
                        print(f"  Values seen so far: {dict(scope_values)}")

        print(f"\n=== Filtering Complete ===")
        print(f"Initial record count: {initial_count}")
        print(f"Active records kept: {active_count}")
        print(f"Non-Active records removed: {removed_count}")
        print(f"\n=== Distribution of ind_bc_scope values ===")
        for value, count in scope_values.most_common():
            print(f"  '{value}': {count} rows")

        # Close and replace original file
        temp_file.close()
        os.replace(temp_path, input_file)
        print(f"\nSuccessfully saved filtered data to {input_file}")
